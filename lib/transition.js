...
isArray,diff,keys,log,map,pick,reverse,some,toArray,uniqBy,omit} = require('./funcs')
...
const addNoTransitionValue = style=>{
  ...
  const transitionKeys = map(pick('property'))(style.transition)
  const noTransitionKey = diff(styleKeys)(transitionKeys)
  const toStandart = key=>assign({},standart,{property:key})
  const transition = comp(map(toStandart),concat(style.transition))
...
module.exports = {needTween,toDefault,splitAll,addTargetValue,addNoTransitionValue}
