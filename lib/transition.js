...
// filterUseless :: transitions:ArrayTransition => obj:Object => transitions:ArrayTransition
const filterUseless = transitions=>obj=>{
  const filterUseless = filter(tra=>{
    const differentCurrentTran = tra.targetValue!==propertyByPath(tra.property)(obj)
    const differentStyledTargetValue = haveDifferentStyledTargetValue(obj)(tra)

    return differentCurrentTran || differentStyledTargetValue
  })

  return filterUseless(transitions)
}
...
module.exports = {needTween,toDefault,splitAll,addTargetValue,
  addNoTransitionValue,isTransition,toTween,filterUseless}
